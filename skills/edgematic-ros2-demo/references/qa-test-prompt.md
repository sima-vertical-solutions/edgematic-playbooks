...
publish_overlay must be true. Stage a finite source longer than the intended
run, verify its duration and content hash, and keep decoder replay disabled.
/image_raw, /detections, and /detections_overlay must all have fresh non-zero
rates. Any JPEG/compressed adapter may subscribe to those real image topics
only. It must not read/replay the MP4, synthesize timestamps, reuse stale
detections, or draw its own overlay. Synthetic replay does not pass VIEW.
...
the rendered frame visibly advances. Soak for at least 30 minutes. Check saved
process identities and fatal logs at 10, 30, 60, and 120 seconds. At 5, 10, 15,
20, 25, and 30 minutes, re-sample detections and the selected compressed overlay,
verify each topic still has exactly its expected publisher, and confirm CMA
headroom has not degraded. Require zero decoder replays and no new kernel CMA,
...
of soak, decoder replay count, CMA start/end values, bridge
listener/subscriptions, and a human-visible advancing frame. Keep credentials,
private addresses, and raw logs out of the reusable report.
