...
3. Reuse an existing Stiga project when its remote and requested revision
   match. Otherwise use `clone_repository` for Stiga and a sibling `sima-core`
   checkout. Never clone either repository onto the board.
4. Call `open_ros_workspace` on Stiga. If it returns a rebind instruction, end
   the turn exactly as instructed; continue from the rebound project on the
